...
pub mod attempt1 {
    ...
    pub fn main() {
        let mut threads = [const { None }; N_THREADS];
...
#[cfg(any())]
pub mod attempt4 {
    use std::{array, cell::Cell, sync::{Mutex, atomic::{AtomicU64, Ordering}}, thread::{self, sleep}, time::Duration};
    ...
    fn thread(my_number: &Mutex<u64>) {
        for _ in 0..N_INCREMENTS {
            ...
            let second_guard = my_number.lock().unwrap();
            *guard += 1;
        }
    }

    /*
    fn foo(takes_a_string: &str) {
        let my_string = String::from("hello");

        std::thread::spawn(|| {
            sleep(Duration::from_secs(5));
            println!("hello from the thread, the string is {my_string}!!!");
        });
    }
    */
