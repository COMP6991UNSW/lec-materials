...
void *run_thread(void *argument) {
    ...
    printf("Thread summing integers %10lu to %11lu finished sum is %lu\n",
           start, finish, sum);
    return NULL;
}
...
int main(int argc, char *argv[]) {
    ...
    unsigned long integers_to_sum = strtol(argv[2], NULL, 0);
    assert(0 < integers_to_sum);

    unsigned long integers_per_thread = (integers_to_sum - 1) / n_threads + 1;
